...
def build_docx_report(packet, settings):
    ...
    title_run = title.add_run("ByteCase Acquire")
    title_run.bold = True
    title_run.font.size = Pt(16)

    subtitle = document.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.add_run("Digital Evidence Acquisition Documentation")

    attribution = document.add_paragraph()
    attribution.alignment = WD_ALIGN_PARAGRAPH.CENTER
    attribution.add_run(f"Part of the {SUITE_NAME} toolset by {PUBLISHER_NAME} | {PRODUCT_DOMAIN}")
    ...
    add_label_value_table(document, [
        ...
        ("Generated By", f"{APP_NAME} - {APP_SUBTITLE} v{APP_VERSION}"),
    ])
